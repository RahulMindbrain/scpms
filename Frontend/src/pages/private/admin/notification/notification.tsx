...
const AdminNotificationPage = () => {
  ...
  const getTagConfig = (type: string) => {
    switch (type) {
      ...
      default: return { label: 'Notification', icon: Bell, color: 'text-slate-600', bg: 'bg-slate-50', border: 'border-slate-100' };
    }
  };
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold tracking-tight text-slate-900">Admin Notifications</h1>
        ...
        <div className="grid gap-4 md:grid-cols-[1fr,300px]">
            ...
            {filterTabs.map((tab) => (
              ...
              >
                ...
                <span className={cn(
                  ...
                  activeFilter === tab.key ? 'bg-indigo-100 text-indigo-800' : 'bg-slate-100 text-slate-500'
                )}>
                  {tab.count}
                </span>
              </button>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-slate-200 bg-white py-2.5 pl-11 pr-4 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 transition-all shadow-sm outline-hidden"
            />
          </div>
        </div>
